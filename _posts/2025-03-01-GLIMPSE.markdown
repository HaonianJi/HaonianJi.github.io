...
website: 
details:
  - Introduced `GLIMPSE`, a benchmark with over 4,300 highly visual-centric questions to evaluate if Large Vision Language Models (LVLMs) truly understand video content.
  - Designed questions that require watching the entire video and reasoning based on context, which cannot be answered from single frames.
  - Revealed that even the best-performing models like Gemini 1.5 Pro struggle, indicating significant room for improvement in LVLMs' video understanding capabilities.
---
